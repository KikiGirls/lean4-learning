# 附录A：Mordell 主定理的详细推导

## A.1 定理表述与基本记号

本附录对正文定理 [thm:mordell-basic] 中使用的类群下降法作较为完整的推导。设 $d<0$ 为无平方因子的整数，且 $d\equiv 2$ 或 $3\pmod 4$。令 $D=\mathbb Z[\sqrt d]$，记 $h(D)$ 为 $D$ 的类数，并假设 $3\nmid h(D)$。在这些假设下，需要证明 Mordell 方程
$$
    y^2=x^3+d
$$
...
## A.3 将方程在 $D=\mathbb Z[\sqrt d]$ 中分解
...
本节只汇总上述推导中较关键的结构性定理。初等整数运算、模运算分类和多项式代入验证不再单列。以下按本节出现顺序编号，便于与正文括注对应。

**定理 T1（Dedekind 域中理想互素判别）**  
在 Dedekind 域中，两个非零理想互素，当且仅当它们没有公共素理想因子。

**定理 T2（素理想在 $\mathbb Z$ 上的收缩）**  
若 $\mathfrak p$ 是整数环扩张中的非零素理想，则 $\mathfrak p\cap\mathbb Z$ 是 $\mathbb Z$ 的非零素理想，因而形如 $(p)$，其中 $p$ 是有理素数。

**定理 T3（数域整数环是 Dedekind 域与理想唯一分解）**  
