\newpage
\setChapterHeader{3}{Mordell 方程相关命题的 Lean 4 形式化实现}
\section{Mordell 方程相关命题的 Lean 4 形式化实现}
\zihao{-4}

本文以 Mordell 方程的整数解问题为典型案例，深入研究复杂数学证明在 Lean 4 中的形式化表达以及 AI 辅助证明开发的方法。选择 Mordell 方程作为研究对象，主要因为该问题兼具充足的数学复杂度与成熟的参考依据：Baanen 等人此前已在 Lean 3 中成功形式化了若干 \(d < 0\) 情形下的求解过程，并确立了基于类群计算与理想下降法的证明路线\cite{Baanen2023MordellLean}。本文将在充分吸收其证明思想的基础上，在 Lean 4 与 Mathlib4 环境中对相关定义、引理和证明目标进行重构与拆解，进而沿着该路线完成形式化证明的开发，并借此全面评估 AI 工具在整个证明构建过程中的辅助价值与局限性。
...
在本文的 Lean 4 证明过程中，核心参考的是 Mordell\cite{Mordell1914} 中一个基于类群下降法得到的存在性结论，用来求 \(d=-1,-2,-5,-6,-13\) 等特殊负整数参数下的整数解。

\subsection{使用的主要结论}

\begin{theorem}[]\label{thm:mordell-basic}
令 \(d<0\) 为无平方因子的整数，满足
\[
...
定理 \ref{thm:mordell-basic} 是本文后续 Lean 4 形式化实践中的核心数学依据。它将原本关于整数点存在性的数论问题，转化为关于参数 \(m\) 的条件判断。在 Lean 4 中，本文将该结论拆成两个方向处理：\texttt{mordell\_d} 证明任意整数解必须满足上述参数化条件；反向的代数恒等式由 \texttt{mordell\_param\_solution} 给出。该结论的证明见附录 A。图 \ref{fig:mordell-math-proof-strategy} 展示了主要证明思路。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{mordell_math_proof_strategy.png}
  \caption{Mordell 方程主要证明思路图}
  \label{fig:mordell-math-proof-strategy}
\end{figure}

\subsection{从元素分解到理想分解}
...
再用 \texttt{factor\_y\_sq\_sub\_d} 把整数等式提升为 \(\mathbb Z[\sqrt d]\) 中的元素等式：
...
第一，环 \(\mathbb Z[\sqrt d]\) 是 Dedekind 整环。
...
它依赖几个中间步骤：首先，由 \(d\) 无平方因子且 \(d\equiv2,3\pmod4\) 可知 \(d\) 不是平方数，因此 \(\mathbb Z[\sqrt d]\) 无零因子。其次，\(\mathbb Z[\sqrt d]\) 作为 \(\mathbb Z\)-模由 \(1\) 与 \(\sqrt d\) 有限生成，因此具有 Noetherian 性。再次，作为 \(\mathbb Z\) 的有限整扩张，其 Krull 维数至多为 \(1\)。最后，平方自由性与模 \(4\) 条件保证 \(\mathbb Z[\sqrt d]\) 正是对应二次域的整数环，从而在其分式域中整闭。由这些性质可得 \(\mathbb Z[\sqrt d]\) 是 Dedekind 整环。
...
\langle y-\sqrt d\rangle
\]
互素。由于在 Lean 里面直接处理二次整数环里的理想互素证明会十分麻烦，本文将这一命题转化为整数中的 Bezout 互素性。考虑理想 \(I+J\)。由于
\[
...
\]
可得 \(1\in I+J\)。
因此 \(I+J=D\)，即 \(\langle y+\sqrt d\rangle\) 与 \(\langle y-\sqrt d\rangle\) 互素。
...
\]
后，还需要利用类数条件把理想三次方下降为元素三次方。数学上，若类群阶数与 \(3\) 互素，则 \([I]^3=1\) 推出 \([I]=1\)，即 \(I\) 是主理想。
...
理想成为主理想后，由两个主理想相等可以得到生成元相差一个单位。因此存在单位 \(u\in D^\times\)，满足
\[
y+\sqrt d=u\alpha^3.
\]
对于 \(d\le -1\)，可以证明 \(\mathbb Z[\sqrt d]\) 中三次幂映射
\[
D^\times\longrightarrow D^\times,\qquad v\longmapsto v^3
\]
...
前面的 \texttt{mordell\_d} 定理仍然依赖一个类数条件：
\[
3\nmid h(\mathbb Z[\sqrt d]).
\]
在 Lean 中这一条件表现为：
...
hcl : Nat.gcd (quadClassNumber d) 3 = 1
\end{leanlisting}
这里 \texttt{quadClassNumber d} 表示二次整数环 \(\mathbb Z[\sqrt d]\) 的类数。因此，若要把抽象的 Mordell 下降定理应用到具体参数 \(d\)，还必须证明相应类数不被 \(3\) 整除。

\subsubsection{\(d=-1\) 与 \(d=-2\) 的情形}
...
\subsubsection{\(d=-5,-6,-13\) 的有限理想类代表}
...
在 Lean 中，这一步由引理 \texttt{exists\_mk0\_eq\_mk0\_of\_approx} 承担。
\begin{leanlisting}
...
这一引理的基础来自 Baanen 等人对 Dedekind 整环和类群的形式化工作\cite{Baanen2021DedekindClassGroups}。该结果在本文中可概括为如下形式。

\begin{theorem}[]\label{thm:classgroup-approx}
...
在形式化时，本文没有把该定理只写成数域整数环上的专用命题，而是把底环推广为带绝对值的欧几里得整环 \(R\)，把 \(\mathcal O_K\) 推广为 Dedekind 整环 \(S\)，并把“\(J\) 整除由 \(M\) 中元素乘积生成的理想”改写为“某个公共倍数 \texttt{prodM} 的像属于 \(J\)”。这是因为后续有限分类实际只需要
\[
(n)\subseteq J
\]
...
有了 \texttt{mordell\_d} 以后，具体 Mordell 方程的求解就转化为有限的整数计算。对于给定负整数 \(d\)，定理给出必要条件：
\[
d=1-3m^2
\quad\text{或}\quad
d=-1-3m^2,
...
y=m(3d+m^2).
\]
因此，求解流程分为三步：第一步代入具体的 \(d\)，求出可能的整数 \(m\)；第二步由公式计算对应的 \(y\)；第三步把 \(y\) 代回原方程，求出 \(x\)。在 Lean 中，前两步主要由 \texttt{omega}、\texttt{nlinarith} 和模运算排除不可能的平方值；最后一步利用整数三次函数的单射性，从 \(x^3=n^3\) 推出 \(x=n\)。

本文在 \texttt{Solutions.lean} 中给出了五个具体参数的求解结果。对应的最终解集如下：

\begin{table}[H]
\centering
\caption{五个具体 Mordell 方程参数的整数解}
\label{tab:mordell-solutions}
\begin{tabular}{ccc}
\toprule
